...
<nav class="navbar navbar-expand-lg navbar-light">
  ...
  <div class="collapse navbar-collapse" id="navbarNav">
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
        <a class="nav-link" href="donorAdmin.php">Donor Item</a></a>
      </li>
      <li class="nav-item">
  ...
  <div class="col-lg-6 col-sm-12">
    <?php
     ...
     $rows = $stmt3->fetchAll(PDO::FETCH_ASSOC);
      echo "<h2 class='shadow-lg p-3 mb-5  rounded mx-auto aaa'>The Overall Donations Are ₹ ".$rows[0]['SUM(donations)']." </h2>" ;
    ?>
  </div>
  <div class="col-3">  </div>
</div>
  ...
  <div class = "row ">
    <div class="col-lg-6 col-sm-12 text-center">
      ...
      <table class="table shadow-lg p-3 mb-5 bg-light rounded ">
        ...
        <tbody class="">
           <?php
             $stmt3 = $pdo->query("SELECT ngo_account.donor_id , donor.name , ngo_account.donations FROM donor JOIN ngo_account WHERE donor.donor_id = ngo_account.donor_id AND ngo_account.city_id = 11");
             $rows = $stmt3->fetchAll(PDO::FETCH_ASSOC);
             $count = 1;
              foreach($rows as $row){
                echo "<tr class=''>";
    ...
    <div class="col-lg-6 col-sm-12  text-center">
      ...
      <table class="table shadow-lg p-3 mb-5 bg-light rounded ">
        ...
        <tbody class="">
           <?php
             $stmt3 = $pdo->query("SELECT ngo_account.donor_id , donor.name , ngo_account.donations FROM donor JOIN ngo_account WHERE donor.donor_id = ngo_account.donor_id AND ngo_account.city_id = 12");
             $rows = $stmt3->fetchAll(PDO::FETCH_ASSOC);
             $count = 1;
              foreach($rows as $row){
                echo "<tr class=''>";
   ...
   <div class = "row">
    <div class="col-lg-6 col-sm-12  text-center">
      ...
      <table class="table shadow-lg p-3 mb-5 bg-light rounded ">
        ...
        <tbody class="">
           <?php
             $stmt3 = $pdo->query("SELECT ngo_account.donor_id , donor.name , ngo_account.donations FROM donor JOIN ngo_account WHERE donor.donor_id = ngo_account.donor_id AND ngo_account.city_id = 13");
             $rows = $stmt3->fetchAll(PDO::FETCH_ASSOC);
             $count = 1;
              foreach($rows as $row){
                echo "<tr class=''>";
    ...
    <div class="col-lg-6 col-sm-12  text-center">
      ...
      <table class="table shadow-lg p-3 mb-5 bg-light rounded ">
        ...
        <tbody class="">
           <?php
             $stmt3 = $pdo->query("SELECT ngo_account.donor_id , donor.name , ngo_account.donations FROM donor JOIN ngo_account WHERE donor.donor_id = ngo_account.donor_id AND ngo_account.city_id = 14");
             $rows = $stmt3->fetchAll(PDO::FETCH_ASSOC);
             $count = 1;
              foreach($rows as $row){
                echo "<tr class=''>";
